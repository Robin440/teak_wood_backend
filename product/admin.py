...
class ProductAdmin(admin.ModelAdmin):
    ...
    # add fields to diplay in user interface as list
    list_display = ('name', 'display_image', 'price','qty' ,'category')  # Add 'display_image' to list_display
    ...
    fieldsets = [
        ('Product Information', {'fields': ['name', 'description', 'price', 'qty', 'category', 'sold_by', 'material', 'brand', 'color', 'size', 'warranty', 'number_of_box', 'features', 'country_of_origin',  'is_featured']}),
        ('Images', {'fields': ['image_one', 'image_two', 'image_three', 'image_four', 'image_five']}),
        ('Video', {'fields': ['video']}),
    ]
    ...
    def display_image(self, obj):
        if obj.image_one:
            return format_html('<img src="{}" style="max-height: 100px; max-width: 100px;" />'.format(obj.image_one.url))
        return "No Image"
